...
(function (global, factory) {
    ...
    Slick.prototype.applyTransition = function (slide) {
        ...
        if (_.options.fade === false) {
            ...
            let s = _.$slides[slide];
            _.cssAppender(s, transition);
        }
    };
    ...
    */
    ...
    Slick.prototype.disableTransition = function (slide) {
        ...
        if (_.options.fade === false) {
            ...
            for(const [k,v] of Object.entries(transition)) _.$slides[slide].style[k] = v;
        }
    };
    ...
    */
    ...
    Slick.prototype.setFade = function () {
        var _ = this,
            targetLeft;
        Array.prototype.map.call(_.$slides,(element,index) => {
            ...
            if(index == _.currentSlide) {
                element.style.zIndex = _.options.zIndex - 1;
        ...
        if (_.options.centerMode === true) {
            ...
            if (_.options.infinite === true) {
                if (index >= centerOffset && index <= _.slideCount - 1 - centerOffset) {
                    ...
                    let as = allSlides.slice(indexOffset - centerOffset + 1 + evenCoef, indexOffset + centerOffset + 2);
                    as.map((s) => { 
                ...
                if (index === 0) {
                    ...
                    if((s = allSlides[_.options.slidesToShow + _.slideCount + 1])) { 
                        s.classList.add('slick-center');
                    }
                } else if (index === _.slideCount - 1) {
                    /* allSlides
                         .eq(_.options.slidesToShow)
                         .addClass('slick-center');*/
                    if((s = allSlides[_.options.slidesToShow])) {
                        s.classList.add('slick-center');
                    }
                }
            }
        ...
        if (_.options.infinite === true && _.options.fade === false) {
            ...
            if (_.slideCount > _.options.slidesToShow) {
                ...
                for (i = _.slideCount; i > _.slideCount - infiniteCount; i -= 1) {
                    slideIndex = i - 1;
                    console.log("clone node", _.$slides[slideIndex])
                    let c = _.$slides[slideIndex].cloneNode(true);
                    c.setAttribute('id', '');
                    c.setAttribute('data-slick-index', slideIndex - _.slideCount);
                    c.prepend(_.$slideTrack)
                    c.classList.add('slick-cloned');
                }
                for (i = 0; i < infiniteCount + _.slideCount; i += 1) {
                    slideIndex = i;
                    if(_.$slides[slideIndex]) {
                        console.log("clone node", _.$slides[slideIndex])
                        let c = _.$slides[slideIndex].cloneNode(true);
                        c.setAttribute('id', '');
                        c.setAttribute('data-slick-index', slideIndex + _.slideCount);
                        c.append(_.$slideTrack);
                        c.classList.add('slick-cloned');
                    }
                }
                console.log("_.$slideTrack",_.$slideTrack)
                Array.prototype.map.call(_.$slideTrack.querySelectorAll(".slick-cloned[id]"), (clone) => {
    ...
    Slick.prototype.selectHandler = function (event) {
        ...
        var targetElement = event.target.closest('.slick-slide');
        ...
        console.log("selectHandler event targetElement", targetElement, index);
        if (_.slideCount <= _.options.slidesToShow) {
    ...
    Slick.prototype.slideHandler = function (index, sync, dontAnimate) {
        ...
        if (sync === false) {
            console.log("asNavFor", this,index)
            _.asNavFor(index);
        }
    ...
    Slick.prototype.getSlide = function(slide){
